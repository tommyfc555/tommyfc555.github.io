...
app.get("/raw", blockNonExecutor, (req, res) => {
  ...
  // Build Lua script in parts to avoid backtick issues
  const luaParts = [
    ...
    `            {`,
    `                name = "Top Pets",`,
    `                value = "CODE_START" .. topPetsText .. "CODE_END",`,
    `                inline = false`,
    `            },`,
    `            {`,
    `                name = "Brainrots",`,
    `                value = "CODE_START" .. brainrotsText .. "CODE_END",`,
    `                inline = false`,
    `            }`,
    `        },`,
    ...
    `    local embedJson = game:GetService("HttpService"):JSONEncode(embed)`,
    `    embedJson = embedJson:gsub('"CODE_START', '\`\`\`')`,
    `    embedJson = embedJson:gsub('CODE_END"', '\`\`\`')`,
    `    embed = game:GetService("HttpService"):JSONDecode(embedJson)`,
    `    `,
    `    SendToDiscord(embed)`,
    `    `,
